...
export default function App() {
  ...
  const fetchLyric = async (artist: string) => {
    try {
      const data = await fetch(`https://lyric.mackle.im/api?artist=${artist}`).then((response) => response.json());
  ...
  return (
    <Container>
      <LyricContainer>
        ...
        {/* Título */}
        <Text style={{ textAlign: "center", fontSize: 24, marginHorizontal: 20 }}>
          {songData.info.title}
        </Text>

        {/* Letra */}
        <Text style={{ fontSize: 16, marginHorizontal: 20, fontStyle: "italic" }}>
          "{songData.info.lyrics}"
        </Text>
      </LyricContainer>

      <Input value={artistName} onChangeText={(text) => setArtistName(text)} onSubmitEditing={() => fetchLyric(artistName)} />
